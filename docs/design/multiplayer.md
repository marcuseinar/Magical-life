...
**Not built today.** A QR code does exist in the app (`TableSheet.svelte`,
rendered by `QrCode.svelte`), but it encodes a link into path 2 below — a
shortcut around typing the short code, scanned by the phone's own camera
app rather than an in-app scanner. It still needs the Cloudflare Worker
reachable. The two-way, no-server exchange described above — offer and
answer both carried by QR, host and joiner scanning each other directly —
is proven only in the spike; it is not wired into the app.
...
## What is actually built today: the manual-code fallback

Path 2 above is shipped and is the default a player actually sees. This
section covers its fallback specifically: paste-a-code, with no server and
no camera, for when the short-code path can't be used.
